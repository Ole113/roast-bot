...
exports.run = async (client, message) => {
    ...
    connection.query(`SELECT * FROM roast_bot_custom_prefix WHERE guildID = "${message.guild.id}";`, function (err, result) {
        ...
        connection.query(`SELECT * FROM roast_bot_on_off WHERE guildID = "${message.guild.id}";`, function (err, result) {
            ...
            if ((message.content.toLowerCase().startsWith(`${prefix}bot on`) || message.content.toLowerCase().startsWith(`${prefix}bot off`)) && !message.member.hasPermission("ADMINISTRATOR")) {
                return message.channel.send("You need to be an admin to turn this command on/off.");
            } else if (message.content.toLowerCase().startsWith(`${prefix}bot on`)) {
                if (err) console.log(err);

                if (result[0].bot) {
                    return message.channel.send(`This command is already on, use *${prefix}bot off* to turn it off.`);
                } else if (!result[0].bot) {
                    update = `UPDATE roast_bot_on_off SET username = "${message.author.username}", bot = "${1}" WHERE guildID = "${message.guild.id}";`;
                    updateBot();
                    return message.channel.send(`Bot command has been turned on, use *${prefix}bot off* to turn it back off.`);
                }
            } else if (message.content.toLowerCase().startsWith(`${prefix}bot off`)) {
                if (err) console.log(err);

                if (!result[0].bot) {
                    return message.channel.send(`This command is already off, use *${prefix}bot on* to turn it on.`)
                } else if (result[0].bot) {
                    update = `UPDATE roast_bot_on_off SET username = "${message.author.username}", bot = "${0}" WHERE guildID = "${message.guild.id}";`;
                    updateBot();
                    return message.channel.send(`Bot command has been turned off, use *${prefix}bot on* to turn it back on.`);
                }
            }
            function updateBot() {
            ...
            if (message.content.toLowerCase() === prefix + "bot help" || message.content.toLowerCase() === "rb!") {
            ...
            if ((message.content.toLowerCase() === prefix + "bot" || message.content.toLowerCase() === "rb!") && botStatus) {
            ...
            } else if ((message.content.toLowerCase() === prefix + "bot" || message.content.toLowerCase() === "rb!") && !botStatus) {
