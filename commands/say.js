...
exports.run = async (message) => {
    ...
    connection.query(`SELECT * FROM roast_bot_custom_prefix WHERE guildID = "${message.guild.id}";`, function (err, result) {
        ...
        connection.query(`SELECT * FROM roast_bot_on_off WHERE guildID = "${message.guild.id}";`, function (err, result) {
            ...
            if (message.content.toLowerCase() === prefix + "say" || message.content.toLowerCase() === prefix + "say  " && onOrOff) {
            ...
            if (message.content.toLowerCase().startsWith(prefix + "say ") && !(message.content.includes(prefix + "say on") || message.content.includes(prefix + "say off")) && onOrOff) {
                const word = message.content;
                const say = word.slice(prefix.length + 4, word.length);
                return message.channel.send(say);
            } else if (message.content.toLowerCase().startsWith(prefix + "say ") && !onOrOff) {
