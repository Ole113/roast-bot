...
exports.run = async (message) => {
	...
	if (message.content.toLowerCase() === "r!" + "off help" || message.content.toLowerCase() === prefix + "on help") {
	...
	onOff.defer.then(() => {
		...
		if (!message.member.hasPermission("ADMINISTRATOR") && (message.content.toLowerCase().startsWith("r!") || message.content.toLowerCase().startsWith("r!" + "on"))) {
			return message.channel.send("Sorry, you need to be an admin to turn this command on/off. <:roast_circle:474755210485563404>");
		}

		if (message.content.toLowerCase().startsWith("r!" + "off")) {
		...
		if (message.content.toLowerCase().startsWith("r!" + "on")) {
