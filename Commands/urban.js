...
exports.run = async (message) => {
    ...
    if (message.content.toLowerCase() == prefixFile.get(key, "prefix") + "urban help") {
    ...
    if (message.content.toLowerCase().startsWith(prefixFile.get(key, "prefix") + "urban") && onOff.get(key, "urban") == "on") {
        let word = message.content;
        let args = word.slice(prefixFile.get(key, "prefix").length + 5, word.length);
        if (args == "") {
    ...
    } else if(message.content.toLowerCase().startsWith(prefixFile.get(key, "prefix") + "urban") && onOff.get(key, "urban") == "off") {
