...
exports.run = async (message) => {
   ...
   if(message.content.toLowerCase() === prefixFile.prefix + "say help") {
    ...
    if (message.content.toLowerCase().startsWith(prefixFile.prefix + "say ") && onOff.get(key, "say") === "on") {
        const word = message.content;
        const say = word.slice(prefixFile.prefix.length + 4, word.length);
        return message.channel.send(say);
    } else if(message.content.toLowerCase().startsWith(prefixFile.prefix + "say ") && onOff.get(key, "say") === "off") {
