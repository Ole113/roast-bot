...
exports.run = async (message) => {
    ...
    if(message.content.toLowerCase() === prefixFile.get(key, "prefix") + "meme help" || message.content.toLowerCase() === "rb!"){
    ...
    if (message.content.toLowerCase().startsWith(prefixFile.get(key, "prefix") + "meme") && onOff.get(key, "meme") === "on" || message.content.toLowerCase() === "rb!") {
        const randomMemes = Math.ceil(Math.random() * 415);
        if (message.content.toLowerCase().startsWith(prefixFile.get(key, "prefix") + "meme #") || message.content.toLowerCase().startsWith("rb!meme #")) {
            let word2 = message.content;
            let number2 = word2.slice(prefixFile.get(key, "prefix").length + 6, word2.length);
    ...
    } else if(message.content.toLowerCase().startsWith(prefixFile.get(key, "prefix") + "meme") && onOff.get(key, "meme") === "off" || message.content.toLowerCase().startsWith("rb!meme")) {
